...
GitHub Actions によって定期的にクラウド実行される自律型ワークフローのスケジュール一覧です（日本時間 JST に補正済）。

| ワークフロー名 | 実行タイミング (日本時間 JST) | 担当エージェント (部門) | 実行プログラム | 主な役割 |
| :--- | :--- | :--- | :--- | :--- |
| **Inbox定期監視フロー** | 毎日 AM 9:00 / PM 17:00 | 運営・総務部門 (Ops) | `tandy_watcher.py` | `Inbox/` に投入された新規の指示ファイルをスキャンし、適切な処理モードを実行。 |
| **AI提案デリバリーフロー** | 毎週月曜日 AM 9:00 | 経営・統括部門 (審査会) | `tandy_proposal.py` | 最新トレンドの収集から5つの経営プランを起案し、審査会議（80点以上可決）を経て正式提案。 |
| **週次整理整頓フロー** | 毎週日曜日 PM 23:00 | 運営・総務部門 (Ops) | `tandy_housekeeping.py` | 日付揺れニュースレターの補正、決定済提案・古いInbox処理済みのアーカイブ退避。 |
